...
import { db, auth } from '../pages/Authentication/firebase'; // Adjust the path according to your Firebase setup
...
const Profile = () => {
      ...
      try {
        ...
        onAuthStateChanged(auth, async (user) => {
          if (user) {
            ...
            if (userEmail) {
              // Create a query to find the document where email matches the user's email
              const facultyCollectionRef = collection(db, 'faculty');
              const q = query(facultyCollectionRef, where('email', '==', userEmail));
  ...
  if (!facultyData) {
    return <div>No data found!</div>; // Handle the case where data is null
  }
  ...
  <div className="relative w-full h-full rounded-full overflow-hidden drop-shadow-2">
    <img
      src={facultyData.profilePicUrl} // Ensure this is a valid URL or path
      alt="profile"
  ...
  <p className="mt-4.5">
    ...
    <strong>Phone:</strong> {facultyData.phone} <br />
